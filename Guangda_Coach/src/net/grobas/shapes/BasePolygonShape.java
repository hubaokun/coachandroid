...
abstract public class BasePolygonShape implements PolygonShape {
	...
	public Path getPolygonPath(PolygonShapeSpec spec) {
		...
		do {
			// next vertex point
			pointX = spec.getCenterX() + spec.getDiameter() / 2f * (float) Math.cos(2 * Math.PI * i / spec.getNumVertex());
			pointY = spec.getCenterY() + spec.getDiameter() / 2f * (float) Math.sin(2 * Math.PI * i / spec.getNumVertex());
			// rotate vertex
			rotatedPointX = (float) (Math.cos(angleRadians) * (pointX - spec.getCenterX()) - Math.sin(angleRadians) * (pointY - spec.getCenterY()) + spec.getCenterX());
			rotatedPointY = (float) (Math.sin(angleRadians) * (pointX - spec.getCenterX()) + Math.cos(angleRadians) * (pointY - spec.getCenterY()) + spec.getCenterY());
